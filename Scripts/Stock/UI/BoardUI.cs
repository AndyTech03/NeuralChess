...
namespace Chess.Game {
	public class BoardUI : MonoBehaviour {
		...
		void CreateBoardUI () {
			...
			for (int rank = 0; rank < 8; rank++) {
				for (int file = 0; file < 8; file++) {
					...
					Destroy(square.gameObject.GetComponent<MeshCollider>());
					if (isHumansBoard)
						square.gameObject.AddComponent<BoxCollider>();
					square.parent = transform;
					square.name = BoardRepresentation.SquareNameFromCoordinate (file, rank);
					square.position = PositionFromCoord (file, rank, 0);
					square.gameObject.isStatic = true;
					...
					pieceRenderer.gameObject.isStatic = true;
		...
		public Vector3 PositionFromCoord (int file, int rank, float depth = 0) {
			if (whiteIsBottom) {
				return new Vector3 (transform.position.x -3.5f + file, transform.position.y - 3.5f + rank, depth);
			}
			return new Vector3 (transform.position.x - 3.5f + 7 - file, transform.position.y + 7 - rank - 3.5f, depth);
