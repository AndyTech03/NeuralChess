namespace Chess {
	public static class PieceSquareTable {
		...
		public static int Read (int[] table, int square, bool isWhite) {
			///Если мои белые инвертируем координаты
			if (isWhite) {
				int file = BoardRepresentation.FileIndex(square); /// строка или столбец ХЗ
				int rank = BoardRepresentation.RankIndex(square); /// строка или столбец ХЗ
				rank = 7 - rank;
				square = rank * 8 + file;
			}
