namespace Chess.Game {
    ...
    public class AIPlayer : Player {
		...
		public NeuralNetwork AIBrain
		{
			get
			{
				NeuralNetwork result = null;
				if (search.evaluation.GetType() == typeof(NetWorkEvaluation))
					result = ((NetWorkEvaluation)search.evaluation).Brain;
				return result;
			}
			set
			{
				if (search.evaluation.GetType() == typeof(NetWorkEvaluation))
					((NetWorkEvaluation)search.evaluation).Brain = value;
			}
		}

        public override string ToString()
        {
			return AIBrain == null ? "AI" : AIBrain.ToString();
        }

        public AIPlayer (Board board, AISettings settings, IEvaluation serchEval = null) {
			this.settings = settings;
			this.board = board;
			search = new Search(board, settings, serchEval);
